...
public class DIKBD {
	...
	public static int C;
	/**
	 * Parameter for the data structure
	 */
	public static int Delta;
	
	private DIKBD_Graph G;
	...
	/**
	 * Creates a new instances of the algorithm
	 * @param g A DIKBD graph
	 * @param source source node
	 */
	public DIKBD(DIKBD_Graph g, int source) {
	...
	private void stepForwardInDist() {
		if (baseAproxBucket!=null) {
			...
			DIKBD_Vertex entrance = baseAproxBucket.getEntrance(); 
	 ...
	 */
	private void insertLow(int ndj, DIKBD_Vertex v){
		lowLevelBuckets[ndj - dL].insertVertexDist(v);
	}
	
	private void moveToLow(int dj, int ndj, DIKBD_Vertex v){
		if(dj<dK){
	 ...
	 * @param v
	 */
	private void insert(int key, int clueKey, int ndj,  DIKBD_Vertex v )
	{
		int pos = posCalculator(key, baseAproxBucket.getKey());
		if(highLevelBuckets[pos] != null){
			highLevelBuckets[pos].insertVertexDist(v);
		}else{
			AproxBucket theNewBucket = new AproxBucket(v, key);
	 ...
	 * @param v vertex being inserted
	 */
	private void firstInsertion(int ndj,DIKBD_Vertex v){
	 ...
	 */
	private boolean deleteToMove(int key , DIKBD_Vertex v){
		int pos = posCalculator(key, baseAproxBucket.getKey());
	 ...
	 */
	private void moveBucketToBucket(int dj, int ndj, DIKBD_Vertex v){
		...
		/*
	public void resetBu564ketMap(int source){
		C= G.getCd();
		double x = (int)(Math.log(Math.sqrt(C))/Math.log(2))+0.0;
		Delta = (int)(Math.pow(2,x));
		Snum = 0;
